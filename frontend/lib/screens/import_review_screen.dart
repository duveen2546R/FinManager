...
class _ImportReviewScreenState extends State<ImportReviewScreen> {
  ...
  static const _expenseCats = {
    'Food', 'Travel', 'Bills', 'Shopping', 'Rent', 'Others'
  };
  ...
  List<Map<String, dynamic>> _parseCsv(String text) {
    ...
    final headers =
        rows.first.map((v) => _normHeader(v.toString())).toList();
    final amountCol = _col(headers,
        ['amount', 'amt', 'value', 'transactionamount', 'price', 'total', 'money']);
    final debitCol = _col(headers, [
      'debit', 'withdrawal', 'withdrawalamt', 'withdrawalamount', 'dr',
      'debitamount', 'moneyout', 'paidout', 'outflow', 'spent'
    ]);
    final creditCol = _col(headers, [
      'credit', 'deposit', 'depositamt', 'depositamount', 'cr',
      'creditamount', 'moneyin', 'paidin', 'inflow', 'received'
    ]);
    ...
    final dateCol = _col(headers, [
      'date', 'transactiondate', 'txndate', 'valuedate', 'posteddate',
      'postingdate', 'datetime', 'bookingdate', 'trandate'
    ]);
    final typeCol = _col(headers, [
      'type', 'transactiontype', 'drcr', 'crdr', 'debitcredit', 'direction',
      'kind'
    ]);
    final categoryCol =
        _col(headers, ['category', 'categories', 'tag', 'tags', 'group', 'head']);
    final titleCol = _col(headers, [
      'title', 'description', 'desc', 'narration', 'particulars', 'details',
      'detail', 'memo', 'notes', 'note', 'name', 'transaction', 'remarks',
      'reference', 'purpose'
    ]);
    final merchantCol = _col(headers,
        ['merchant', 'payee', 'vendor', 'beneficiary', 'party', 'counterparty']);
    ...
    for (final row in rows.skip(1)) {
      ...
      final title = _firstNonEmpty([cell(row, titleCol), cell(row, merchantCol)]) ??
          'Imported transaction';
      final merchant = _firstNonEmpty([cell(row, merchantCol), cell(row, titleCol)]);
      final category = _mapCategory(
          cell(row, categoryCol), type, '$title ${merchant ?? ''}');
  ...
  static String _inferType(String rawType,
      {double? signedAmount, bool debit = false, bool credit = false}) {
    if (credit && !debit) return 'Income';
    if (debit && !credit) return 'Expense';
    final t = rawType.toLowerCase().trim();
    if (RegExp(r'income|credit|^cr$|deposit|inflow|received|refund|salary')
        .hasMatch(t)) {
      return 'Income';
    }
    if (RegExp(r'expense|debit|^dr$|withdraw|outflow|payment|paid|spent|purchase')
        .hasMatch(t)) {
      return 'Expense';
    }
  ...
  static String _mapCategory(String raw, String type, String context) {
    ...
    if (type == 'Income') {
      if (RegExp(r'salary|payroll|wage|stipend').hasMatch(text)) return 'Salary';
      if (RegExp(r'bonus|incentive').hasMatch(text)) return 'Bonus';
      if (RegExp(r'gift|reward|cashback').hasMatch(text)) return 'Gift';
      if (RegExp(r'interest|dividend|investment|return|maturity').hasMatch(text)) {
        return 'Investment';
      }
      return 'Others';
    }
    if (RegExp(r'food|restaurant|cafe|coffee|grocer|dining|snack|swiggy|zomato|meal|lunch|dinner|breakfast|pizza|bakery')
        .hasMatch(text)) {
      return 'Food';
    }
    if (RegExp(r'travel|uber|ola|taxi|cab|train|flight|bus|fuel|petrol|diesel|metro|irctc|transport|toll|parking')
        .hasMatch(text)) {
      return 'Travel';
    }
    if (RegExp(r'bill|electric|water|gas|internet|wifi|phone|mobile|recharge|dth|utility|broadband|insurance|emi|loan|subscription')
        .hasMatch(text)) {
      return 'Bills';
    }
    if (RegExp(r'shop|amazon|flipkart|myntra|cloth|movie|entertain|store|mall|gadget|electronic|apparel|fashion')
        .hasMatch(text)) {
      return 'Shopping';
    }
  ...
  static String? _parseDate(String raw) {
    ...
    const patterns = [
      ...
      'd MMM yyyy', 'dd MMM yyyy', 'MMM d yyyy', 'MMMM d yyyy', 'd-MMM-yyyy',
      'dd-MMM-yyyy', 'dd/MM/yy', 'MM/dd/yy', 'yyyyMMdd'
    ];
